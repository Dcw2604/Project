...
import {
  Box, Paper, TextField, Button, Typography, Avatar, Chip,
  CircularProgress, Alert, Card, CardContent, 
  LinearProgress, IconButton, Tooltip, Grid, Stack
} from '@mui/material';
import {
  Send, Lightbulb, Psychology, MenuBook, TipsAndUpdates, 
  Timer, Assessment, Person, SmartToy
} from '@mui/icons-material';

// API Configuration
const API_BASE_URL = 'http://127.0.0.1:8000/api';
const getAuthHeaders = () => ({
  'Authorization': `Token ${localStorage.getItem('token') || localStorage.getItem('authToken')}`,
...
const ChatAlgorithmTest = () => {
  ...
  const loadAvailableExams = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addTutorMessage('❌ Failed to load available exams. Please try again.', 'error');
      }
    } catch (error) {
      console.error('Error loading exams:', error);
      addTutorMessage('❌ Error connecting to server. Please check your connection.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const startExamSession = async (examId) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/exam-sessions/${examId}/start/`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
      ...
      if (response.ok) {
        ...
        if (examData.questions && examData.questions.length > 0) {
          ...
          addTutorMessage("❌ This exam has no questions available. Please contact your teacher.", 'error');
        }
      } else {
    ...
    if (question.options && question.options.length > 0) {
      ...
      if (validOptions.length > 0) {
        ...
        validOptions.forEach((option, index) => {
          questionMessage.text += `${String.fromCharCode(65 + index)}. ${option}\n`;
        });
  ...
  const generateContextualHints = (question, hintLevel) => {
    ...
    return hints[hintLevel] || "You're very close! Think about the most important information that was emphasized in the study material.";
  };
    ...
    try {
      ...
      if (evaluation.isCorrect) {
        ...
        setStudentProgress(prev => ({
          ...
          score: Math.round(((prev.correctAnswers + 1) / (prev.questionsCompleted + 1)) * 100)
        }));
        ...
        if (questionAttempts >= maxAttempts) {
          ...
          setStudentProgress(prev => ({
            ...prev,
            questionsCompleted: prev.questionsCompleted + 1,
            score: Math.round((prev.correctAnswers / (prev.questionsCompleted + 1)) * 100)
          }));
          ...
          const feedbackMessage = {
            id: Date.now(),
            text: `🤔 **That's not quite right, but you're thinking!**\n\n**What you might be considering:** ${evaluation.commonMistake}\n\n**Think about this:** ${evaluation.guidance}\n\n💪 **Attempts remaining:** ${maxAttempts - questionAttempts}\n\n${hintLevel < maxHints ? "💡 Would you like a hint, or do you want to try again?" : "🔄 Give it another try!"}`,
            sender: 'tutor',
            timestamp: new Date().toISOString(),
            type: 'feedback'
          };
      ...
      addTutorMessage("❌ Sorry, I had trouble evaluating your answer. Please try again.", 'error');
    } finally {
  ...
  const evaluateStudentAnswer = (question, studentAnswer) => {
    ...
    const isCorrect = studentAnswerClean === correctAnswerClean ||
                     studentAnswerClean.includes(correctAnswerClean) ||
                     (question.options && studentAnswerClean.match(/^[a-d]$/i) && 
                      question.options[studentAnswerClean.charCodeAt(0) - 97] === correctAnswer);
  ...
  const moveToNextQuestion = () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      setTimeout(() => {
        addTutorMessage(`📖 Great! Let's continue building your knowledge with the next question.`);
        setTimeout(() => {
  ...
  const completeKnowledgeTest = () => {
    ...
    const finalScore = Math.round((studentProgress.correctAnswers / questions.length) * 100);
    const performance = finalScore >= 80 ? 'Outstanding' : 
                       finalScore >= 70 ? 'Good' : 
                       finalScore >= 60 ? 'Satisfactory' : 'Needs Improvement';
    
    const completionMessage = {
      id: Date.now(),
      text: `🎓 **Knowledge Assessment Complete!**\n\n**Your Performance Summary:**\n\n📊 **Score:** ${finalScore}% (${performance})\n✅ **Correct Answers:** ${studentProgress.correctAnswers}/${questions.length}\n💡 **Hints Used:** ${studentProgress.hintsUsed}\n🔄 **Total Attempts:** ${studentProgress.totalAttempts}\n\n**What this means:**\n${generatePerformanceAnalysis(finalScore)}\n\n**Next Steps:**\n${generateRecommendations(finalScore)}\n\n🌟 Remember: Every question you answered helped reinforce your understanding of "${examSession.title}". Keep up the great work!`,
      sender: 'tutor',
      timestamp: new Date().toISOString(),
      type: 'completion'
    };
  ...
  const handleTimeUp = () => {
    setTimerActive(false);
    addTutorMessage("⏰ Time's up! Let's see how you did on the questions you completed.", 'info');
    completeKnowledgeTest();
  };
  ...
  if (showExamSelection) {
    return (
      <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', p: 3 }}>
        <Typography variant="h4" gutterBottom sx={{ color: 'white', textAlign: 'center', mb: 1 }}>
          🧠 Knowledge Assessment Center
        </Typography>
        
        <Typography variant="subtitle1" sx={{ color: 'rgba(255,255,255,0.8)', textAlign: 'center', mb: 4 }}>
          Choose an assessment to test your knowledge. I'll guide you through the questions and help you learn!
        </Typography>
        ...
        {loading ? (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, maxWidth: 900, mx: 'auto' }}>
            {availableExams.length > 0 ? (
                ...
                <Card key={exam.id} sx={{ 
                  ...
                  <CardContent sx={{ p: 3 }}>
                    <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                      <Box flex={1}>
                        <Typography variant="h5" sx={{ color: 'white', mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                          <Assessment /> {exam.title}
                        </Typography>
                        <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.8)', mb: 2 }}>
                          {exam.description || 'Test your knowledge and understanding of key concepts'}
                        </Typography>
                      </Box>
                    </Box>
                    ...
                    <Grid container spacing={2} alignItems="center">
                      <Grid item xs={12} sm={6}>
                        <Box display="flex" gap={1} flexWrap="wrap">
                          <Chip 
                            icon={<Assessment />}
                            label={`${exam.total_questions || exam.num_questions || 0} Questions`} 
                            size="small" 
                            sx={{ background: 'rgba(33, 150, 243, 0.8)', color: 'white' }} 
                          />
                          <Chip 
                            icon={<Timer />}
                            label={`${exam.duration_minutes || Math.round((exam.time_limit_seconds || 3600)/60)} min`} 
                            size="small" 
                            sx={{ background: 'rgba(76, 175, 80, 0.8)', color: 'white' }} 
                          />
                          {exam.difficulty_level && (
                            <Chip 
                              label={exam.difficulty_level} 
                              size="small" 
                              sx={{ background: 'rgba(255, 152, 0, 0.8)', color: 'white' }} 
                            />
                          )}
                        </Box>
                      </Grid>
              ...
              >
                <Typography variant="h6" gutterBottom>No Assessments Available</Typography>
                <Typography>
                  Your teacher hasn't created any knowledge assessments yet. Check back later or contact your teacher for more information.
                </Typography>
              </Alert>
            )}
          </Box>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Box>
            ...
            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>
              Question {currentQuestionIndex + 1} of {questions.length} • Score: {studentProgress.score}%
          ...
          <Box display="flex" gap={1}>
            <Tooltip title={`Get hint (${maxHints - hintLevel} remaining)`}>
              <IconButton 
                onClick={provideHint} 
                disabled={!currentQuestion || hintLevel >= maxHints || questionAttempts >= maxAttempts}
                sx={{ 
                  color: hintLevel >= maxHints ? 'rgba(255,255,255,0.3)' : 'white',
                  background: hintLevel >= maxHints ? 'none' : 'rgba(255,255,255,0.1)'
                }}
              >
                <Lightbulb />
              </IconButton>
        ...
        <Box display="flex" gap={2} alignItems="center">
          ...
          <Box textAlign="center" minWidth={80}>
            ...
            <Typography variant="h6" sx={{ color: '#4caf50' }}>
              {studentProgress.correctAnswers}/{studentProgress.questionsCompleted}
            </Typography>
          </Box>
        </Box>
      </Paper>
              ...
              >
                <Avatar
                  sx={{
                    width: 40,
                    height: 40,
                    bgcolor: message.sender === 'student' 
                      ? 'primary.main' 
                ...
                <Paper
                  sx={{
                    ...
                    color: 'white',
                    backdropFilter: 'blur(10px)',
                    border: message.type === 'success' ? '2px solid #4caf50' :
                            message.type === 'hint' ? '2px solid #ff9800' :
                            message.type === 'error' ? '2px solid #f44336' :
                            message.type === 'question' ? '2px solid #2196f3' : 
      ...
      <Paper sx={{ 
        ...
        <Box display="flex" gap={2} alignItems="flex-end">
          <TextField
            ...
            placeholder={
              !currentQuestion ? "Assessment not started..." :
              questionAttempts >= maxAttempts ? "Moving to next question..." :
              hintLevel < maxHints ? "Type your answer or 'hint' for help..." : 
              "Type your answer..."
            }
            disabled={isProcessing || !currentQuestion || questionAttempts >= maxAttempts}
            sx={{
          ...
          <Button
            ...
            disabled={!currentInput.trim() || isProcessing || !currentQuestion || questionAttempts >= maxAttempts}
            sx={{
              minWidth: 60,
              height: 56,
              background: 'linear-gradient(45deg, #2196F3 30%, #21CBF3 90%)',
          ...
          >
            {isProcessing ? <CircularProgress size={24} sx={{ color: 'white' }} /> : <Send />}
          </Button>
        </Box>
