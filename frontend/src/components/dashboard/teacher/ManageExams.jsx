...
const ManageExams = () => {
  ...
  useEffect(() => {
    if (activeTab === 1) { // Only load when on "Manage" tab
      loadExams();
    }
  }, [activeTab]);
  ...
  const loadExamDetails = async (examId) => {
    try {
      ...
      const response = await fetch(`/api/exam-sessions/${examId}/details/`, {
        headers: {
      ...
      {loading ? (
        ...
        <GlassCard>
          <CardContent sx={{ textAlign: 'center', p: 4 }}>
            <Quiz sx={{ fontSize: '4rem', color: 'rgba(255,255,255,0.5)', mb: 2 }} />
        ...
        <DialogContent sx={{ mt: 1 }}>
          {loadingDetails ? (
            <Box display="flex" justifyContent="center" alignItems="center" p={3}>
              <CircularProgress sx={{ color: 'white' }} />
              <Typography sx={{ ml: 2, color: 'white' }}>Loading exam details...</Typography>
            </Box>
          ) : examDetails ? (
            <Box>
              {/* Exam Information */}
                ...
                <CardContent>
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={6} sm={3}>
                      ...
                      <Typography variant="body1" sx={{ color: 'white' }}>{examDetails.exam_session.title}</Typography>
                    </Grid>
                    <Grid item xs={6} sm={3}>
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>Questions:</Typography>
                      <Typography variant="body1" sx={{ color: 'white' }}>{examDetails.exam_session.total_questions}</Typography>
                    </Grid>
                    <Grid item xs={6} sm={3}>
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>Time Limit:</Typography>
                      <Typography variant="body1" sx={{ color: 'white' }}>{Math.round(examDetails.exam_session.time_limit_seconds / 60)} minutes</Typography>
                    </Grid>
                    <Grid item xs={6} sm={3}>
                      <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)' }}>Status:</Typography>
                      <Chip 
                        label={examDetails.exam_session.is_published ? 'Published' : 'Draft'} 
                        color={examDetails.exam_session.is_published ? 'success' : 'default'}
                        size="small"
                      />
                    </Grid>
                    {examDetails.exam_session.description && (
                      <Grid item xs={12}>
                        ...
                        <Typography variant="body1" sx={{ color: 'white' }}>{examDetails.exam_session.description}</Typography>
                      </Grid>
                    )}
                  </Grid>
                </CardContent>
              ...
              <Card sx={{ 
                ...
                <CardContent>
                  ...
                  {examDetails.questions.length > 0 ? (
                      ...
                      {examDetails.questions.map((question, index) => (
                        <Card key={question.id} sx={{ 
                          ...
                          <CardContent>
                            ...
                            <Typography variant="body1" paragraph sx={{ color: 'white', lineHeight: 1.6 }}>
                            ...
                            {(question.option_a || question.option_b || question.option_c || question.option_d) && (
                              <Box sx={{ 
                                ...
                                <Typography variant="subtitle2" gutterBottom sx={{ color: 'rgba(255,255,255,0.8)' }}>
                                  Options:
                                </Typography>
                                {question.option_a && (
                                  <Typography variant="body2" sx={{ 
                                    ...
                                    p: 1,
                                    borderRadius: 1,
                                    background: question.correct_answer === question.option_a ? 'rgba(76, 175, 80, 0.2)' : 'transparent'
                                  }}>
                                    <strong style={{ marginRight: '8px' }}>A.</strong> {question.option_a}
                                    {question.correct_answer === question.option_a && 
                                      <Chip label="✓ Correct" size="small" sx={{ ml: 1, background: 'rgba(76, 175, 80, 0.8)', color: 'white' }} />
                                    }
                                  </Typography>
                                )}
                                {question.option_b && (
                                  <Typography variant="body2" sx={{ 
                                    ...
                                    p: 1,
                                    borderRadius: 1,
                                    background: question.correct_answer === question.option_b ? 'rgba(76, 175, 80, 0.2)' : 'transparent'
                                  }}>
                                    <strong style={{ marginRight: '8px' }}>B.</strong> {question.option_b}
                                    {question.correct_answer === question.option_b && 
                                      <Chip label="✓ Correct" size="small" sx={{ ml: 1, background: 'rgba(76, 175, 80, 0.8)', color: 'white' }} />
                                    }
                                  </Typography>
                                )}
                                {question.option_c && (
                                  <Typography variant="body2" sx={{ 
                                    ...
                                    p: 1,
                                    borderRadius: 1,
                                    background: question.correct_answer === question.option_c ? 'rgba(76, 175, 80, 0.2)' : 'transparent'
                                  }}>
                                    <strong style={{ marginRight: '8px' }}>C.</strong> {question.option_c}
                                    {question.correct_answer === question.option_c && 
                                      <Chip label="✓ Correct" size="small" sx={{ ml: 1, background: 'rgba(76, 175, 80, 0.8)', color: 'white' }} />
                                    }
                                  </Typography>
                                )}
                                {question.option_d && (
                                  <Typography variant="body2" sx={{ 
                                    ...
                                    p: 1,
                                    borderRadius: 1,
                                    background: question.correct_answer === question.option_d ? 'rgba(76, 175, 80, 0.2)' : 'transparent'
                                  }}>
                                    <strong style={{ marginRight: '8px' }}>D.</strong> {question.option_d}
                                    {question.correct_answer === question.option_d && 
                                      <Chip label="✓ Correct" size="small" sx={{ ml: 1, background: 'rgba(76, 175, 80, 0.8)', color: 'white' }} />
                                    }
                                  </Typography>
                                )}
                    ...
                    >
                      This exam has no questions linked to it. This might be due to an error during exam creation.
                    </Alert>
                  )}
                </CardContent>
              </Card>
