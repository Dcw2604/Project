...
const TeacherDashboard = () => {
  ...
  const [recentActivity, setRecentActivity] = useState([
    { id: 1, type: 'lesson', student: 'Sarah Johnson', subject: 'Mathematics', time: '10:00 AM', status: 'completed' },
    { id: 2, type: 'request', student: 'Mike Chen', subject: 'Physics', time: '2:30 PM', status: 'pending' },
    { id: 3, type: 'document', name: 'Algebra Worksheet.pdf', time: '9:15 AM', status: 'uploaded' },
    { id: 4, type: 'lesson', student: 'Emma Davis', subject: 'Chemistry', time: '3:00 PM', status: 'scheduled' }
  ]);
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              <Stack spacing={2}>
                {recentActivity.map((activity) => (
                  <Box key={activity.id} sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(255, 255, 255, 0.05)' }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                      <Box>
                        <Typography variant="body1" fontWeight={600}>
                          {activity.type === 'lesson' ? `Lesson with ${activity.student}` :
                           activity.type === 'request' ? `New request from ${activity.student}` :
                           `Document: ${activity.name}`}
                        </Typography>
                        <Typography variant="body2" sx={{ opacity: 0.7 }}>
                          {activity.type !== 'document' ? activity.subject + ' • ' : ''}{activity.time}
                        </Typography>
                      </Box>
                      <Chip 
                        ...
                        size="small"
                        sx={{ 
                          bgcolor: activity.status === 'completed' ? '#10B981' :
                                  activity.status === 'pending' ? '#F59E0B' :
                                  activity.status === 'scheduled' ? '#3B82F6' : '#8B5CF6',
                          color: 'white'
                        }}
                      />
                    </Stack>
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              <Stack spacing={1}>
                <Box display="flex" justifyContent="space-between">
                  ...
                  <Typography variant="body2" fontWeight={600}>{dashboardData.weeklyHours}h</Typography>
                </Box>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="body2">Average Rating</Typography>
                  <Typography variant="body2" fontWeight={600}>⭐ {dashboardData.averageRating}</Typography>
                </Box>
                <Box display="flex" justifyContent="space-between">
                  <Typography variant="body2">Completion Rate</Typography>
                  <Typography variant="body2" fontWeight={600}>{dashboardData.completionRate}%</Typography>
                </Box>
              </Stack>
            </CardContent>
          </GlassCard>
  ...
  return (
    <StyledContainer>
      {/* App Bar */}
      <AppBar position="static" sx={{ bgcolor: 'rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(20px)' }}>
        <Toolbar>
      ...
      {/* Navigation Tabs */}
      <Box sx={{ borderBottom: 1, borderColor: 'rgba(255, 255, 255, 0.2)', bgcolor: 'rgba(255, 255, 255, 0.05)' }}>
        <Tabs 
          value={activeTab} 
