...
class ApiClient {
  ...
  async uploadDocument(file: File): Promise<UploadResponse> {
    ...
    const response = await this.fetchJson<UploadResponse>('/api/documents/upload/', {
      method: 'POST',
      body: formData,
    })
    ...
    if (response.success && response.doc_id) {
      ...
      console.log('Adding to session uploads:', { label, document_id: response.doc_id })
      schemaRegistry.addSessionUpload(label, response.doc_id)
      console.log('Session uploads after add:', schemaRegistry.getSessionUploads())
    } else {
  ...
  async createExam(payload: CreateExamRequest): Promise<CreateExamResponse> {
    const response = await this.fetchJson<CreateExamResponse>('/api/exams/create/', {
      method: 'POST',
  ...
  async listExamsForStudent(): Promise<ExamCard[]> {
    // First try session exams (created in this session)
  ...
  async startExam(examId: string | number, studentId: number): Promise<StartExamResponse> {
  ...
  async submitAnswers(
    ...
    return this.fetchJson<SubmitAnswersResponse>(`/api/exams/${examId}/submit/`, {
      method: 'POST',
      body: JSON.stringify(payload),
    })
  }

  async finishExam(examId: string | number, examSessionId: number): Promise<FinishExamResponse> {
