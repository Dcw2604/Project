...
import { api } from './api'
import type { DocumentOption, ExamCard, CreateExamRequest, SubmitAnswersRequest } from './types'
...
export function useStartExam() {
  return useMutation({
    mutationFn: ({ examId, studentId }: { examId: string | number; studentId: number }) =>
...
export function useSubmitAnswers() {
  return useMutation({
    mutationFn: ({ examId, payload }: { examId: string | number; payload: SubmitAnswersRequest }) =>
...
export function useFinishExam() {
  return useMutation({
    mutationFn: ({ examId, examSessionId }: { examId: string | number; examSessionId: number }) =>
...
export function useExamRunner(examId: string | number) {
  ...
  return {
    ...
    submitAnswers,
    finishExam,
    isLoading: startExam.isPending || questions.isLoading || submitAnswers.isPending || finishExam.isPending,
    error: startExam.error || questions.error || submitAnswers.error || finishExam.error,
  }
}
